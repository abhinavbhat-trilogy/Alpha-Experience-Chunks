...
import { ChevronRight, Edit2, Trash2, Plus, Download } from 'lucide-react';
    ...
    stages: [
      {
        ...
        userIntent: "We want real-time, accurate student roster data",
      ...
      },
      {
        ...
        userIntent: "We want students to have correct platform access automatically",
      ...
      },
      {
        ...
        userIntent: "We want unified enrollment tracking across all Alpha programs",
      ...
      },
      {
        ...
        userIntent: "We want partner schools to access 2HL learning tools seamlessly",
      ...
      },
      {
        ...
        userIntent: "We want reliable bidirectional data flow with validation",
      ...
      },
      {
        ...
        userIntent: "We want real-time, accurate reporting without manual work",
        currentState: "80+ Google Sheets manually maintained. Data copied from SIS, often stale. Separate dashboards per platform.",
        futureState: "Unified reporting layer. Real-time dashboards. Automated Google Sheets reports. Compliance reports with one click. Executive dashboards.",
        owner: { product: "TBD", tech: "TBD" }
      }
    ]
  }
...
export default function ChunksEditor() {
  ...
  const updateStage = (userId, stageId, field, value) => {
    setData(prev => ({
      ...prev,
      [userId]: {
        ...prev[userId],
        stages: prev[userId].stages.map(stage =>
          stage.id === stageId
            ? { ...stage, [field]: value }
            : stage
        )
  ...
  const updateOwner = (userId, stageId, ownerType, value) => {
    setData(prev => ({
      ...prev,
      [userId]: {
        ...prev[userId],
  ...
  const addStage = (userId) => {
    const newId = `${userId[0]}${data[userId].stages.length + 1}`;
    const isInternal = data[userId].type === 'internal';
    const defaultName = isInternal ? "New Experience" : "New Experience";
    setData(prev => ({
      ...prev,
      [userId]: {
        ...prev[userId],
  ...
  const deleteStage = (userId, stageId) => {
    if (window.confirm("Are you sure you want to delete this experience?")) {
      setData(prev => ({
        ...prev,
        [userId]: {
          ...prev[userId],
          stages: prev[userId].stages.filter(stage => stage.id !== stageId)
        }
      }));
  ...
  const exportData = () => {
    let md = '# EduCRM Chunks - Ownership Structure\n\n';
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">EduCRM Chunks - Interactive Editor</h1>
          <p className="text-gray-600 text-sm">Define ownership structure by user experiences</p>
          <button onClick={exportData} className="mt-3 px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 flex items-center gap-2 text-sm">
            ...
            {selectedUser ? (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => addStage(selectedUser)} className="px-2 py-1 text-xs bg-gray-900 text-white rounded hover:bg-gray-800 flex items-center gap-1">
                    <Plus size={14} /> Add
                  </button>
                </div>
                <div className="space-y-2">
                  {data[selectedUser].stages.map((stage, idx) => {
                    const colors = colorMap[data[selectedUser].color];
                    const isSelected = selectedStage?.id === stage.id;
                    return (
                      <button
                        key={stage.id}
                        onClick={() => { setSelectedStage(stage); setEditMode(false); }}
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setEditMode(!editMode)} className="px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center gap-1">
                      <Edit2 size={12} /> {editMode ? 'Done' : 'Edit'}
                    </button>
                    <button onClick={() => deleteStage(selectedUser, selectedStage.id)} className="px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 flex items-center gap-1">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {editMode ? (
                      <input type="text" value={selectedStage.name} onChange={(e) => updateStage(selectedUser, selectedStage.id, 'name', e.target.value)}
                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                    ) : (
                      <div className="text-lg font-semibold">{selectedStage.name}</div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {editMode ? (
                      <input type="text" value={selectedStage.userIntent} onChange={(e) => updateStage(selectedUser, selectedStage.id, 'userIntent', e.target.value)}
                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
                        placeholder="I want to..." />
                    ) : (
                      <div className="text-sm text-gray-700 bg-purple-50 p-3 rounded-lg italic">"{selectedStage.userIntent}"</div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {editMode ? (
                      <textarea value={selectedStage.currentState} onChange={(e) => updateStage(selectedUser, selectedStage.id, 'currentState', e.target.value)}
                        rows={3} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                    ) : (
                      <div className="text-sm text-gray-700 bg-gray-50 p-3 rounded-lg">{selectedStage.currentState}</div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {editMode ? (
                      <textarea value={selectedStage.futureState} onChange={(e) => updateStage(selectedUser, selectedStage.id, 'futureState', e.target.value)}
                        rows={3} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                    ) : (
                      <div className="text-sm text-gray-700 bg-blue-50 p-3 rounded-lg">{selectedStage.futureState}</div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      {editMode ? (
                        <input type="text" value={selectedStage.owner.product} onChange={(e) => updateOwner(selectedUser, selectedStage.id, 'product', e.target.value)}
                          className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                      ) : (
                        <div className="px-3 py-2 text-sm bg-gray-100 rounded-lg">{selectedStage.owner.product}</div>
                      )}
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1.5">Tech Lead</label>
                      {editMode ? (
                        <input type="text" value={selectedStage.owner.tech} onChange={(e) => updateOwner(selectedUser, selectedStage.id, 'tech', e.target.value)}
                          className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                      ) : (
                        <div className="px-3 py-2 text-sm bg-gray-100 rounded-lg">{selectedStage.owner.tech}</div>
                      )}
                    </div>
                  </div>
                </div>
